...
all_genes_stack %>% filter(gene != "TP53") %>% pull(perc) %>% summary()
...
density_plot <- density_plot + theme(plot.margin = margin(0.5, 0, 0, 0, "cm"))
diff_plot <- diff_plot + theme(plot.margin = margin(0.5, 0, 0, 0, "cm"))
...
#Figure1_A_plots <- ggarrange(Figure1A, density_plot, diff_plot, nrow=3, labels = c("A", "B", "C"))
