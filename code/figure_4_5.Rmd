...
vc_cols = c("#000000", "#966919", "#e7cba9", "#7c9bb9", "#887ecb", "#aa0011", "#0059b2", "#c96552", "#6c8c86", "#C9D487")
names(vc_cols) = c('Age', 'UV', 'HRD', 'Chemotherapy', 'BER', 'POLE',  'APOBEC', 'Tobacco', 'Artifact', 'Immunosuppressants')
...
# Figure 4A - Plot frequency of individuals with at least 10 mutations attributed to a given signature 
...
Freq_comparison_plot <- 
  ...
  filter(eitiology != "MMRD") %>%
  mutate(order_var = ifelse(study == "Schildkraut-B", round(perc), 0)) %>%
  mutate(perc_label = round(perc)) %>%
  mutate(eitiology = factor(eitiology, levels = c("Age", "Artifact", "HRD", "Chemotherapy", "APOBEC", "POLE", "UV", "Immunosuppressants", "BER", "Tobacco", "AA", "Colibactin"))) %>%
  ggplot(., aes(x = eitiology, y = perc_label)) +
  ...
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, color = "black", size = 10, face = "bold"), 
        axis.text.y = element_text(color = "black", size = 10), 
        axis.title.y = element_text(color = "black", size = 10, hjust = .5, face = "bold"), 
        legend.position = c(.9, .85),
        legend.background=element_rect(fill = alpha("white", 0)), 
        legend.text=element_text(size=10, face = "bold")
        ) +
  xlab("") +
  ylab("Percent With Signature") +
  guides(
    fill = "none",  # Hide the fill legend
    pattern = guide_legend(title = NULL)  # Display the pattern legend without a title
  ) +
  geom_text(aes(x = eitiology, y = perc_label, label = perc_label, fill = eitiology, pattern=study),
...
## TMB Plot Overall - Figure 4B

```{r}

sbs_tissue_black_merge <- sbs_tissue_black %>% mutate(study = "Schildkraut-B") %>% rename(perc = perc_sbs_tissue_black_exclude_mmrd) %>% select(c(eitiology, perc, study))
sbs_tcga_white_merge <- sbs_tcga_white_nature %>% mutate(study = "TCGA-W") %>% rename(perc = perc_sbs_tcga_white_nature_exclude_mmrd) %>% select(c(eitiology, perc, study))
...
tmb_plot <- 
  ...
  mutate(eitiology = ifelse(is.na(eitiology), "Not Available", eitiology),
         eitiology = ifelse(eitiology == "Lymphoid", "Not Available", eitiology),
         eitiology = factor(eitiology, levels = c("Not Available", "Tobacco", "BER", "Immunosuppressants", "UV", "POLE", "APOBEC", "Chemotherapy", "HRD", "Artifact", "Age")), 
  ...
  theme_bw() +
  theme(axis.text.y = element_text(face = "bold", size = 10, color= "black"), axis.text.x = element_text(size = 10, color= "black"), 
        axis.title.x = element_text(size = 10, color = "black", vjust = 0.5, face = "bold"), legend.position = "none") +
  xlab("TMB %") +
  ylab("") +
  guides(fill = "none")
...
df_prepare <- 
  ...
  filter(!signature %in% c('SBS6', 'SBS14', 'SBS15', 'SBS20', 'SBS21', 'SBS26', 'SBS44')) %>%
  filter(., activity > 0)
total_activity <- 
  df_prepare %>%
  group_by(Samples) %>% 
...
Freq_comparison_plot_2 <- Freq_comparison_plot + theme(plot.margin = margin(0.5, 0, 0, 0.5, "cm"))
tmb_plot_2 <- tmb_plot + theme(plot.margin = margin(0.5, 0, 0, 0.5, "cm"))
cosmic_counts_tissue_black_2 <- cosmic_counts_tissue_black + theme(plot.margin = margin(0.5, 0, 0, 0.5, "cm"))
cosmic_counts_tcga_white_2 <- cosmic_counts_tcga_white + theme(plot.margin = margin(0.5, 0, 0, 0.5, "cm"))

figure3c <- ggarrange(cosmic_counts_tissue_black_2 + rremove("ylab") + rremove("xlab"), 
                      cosmic_counts_tcga_white_2 + rremove("ylab") + rremove("xlab"), 
                      nrow = 2)

figure3c <- annotate_figure(figure3c, left = textGrob(expression(bold("Mutation Count")), rot = 90, vjust = 1, gp = gpar(cex = 1.3, fontsize = 8)),
                    bottom = textGrob(expression(bold("Samples")), gp = gpar(cex = 1.3, fontsize = 8)))

Figure_4_alt <- ggarrange(Freq_comparison_plot_2, tmb_plot_2, figure3c,
                    labels = c("A", "B", "C"), heights = c(1.75, 1, 2), ncol = 1, nrow = 3)

tiff(file.path(directory, "results/figures/Figure_4_cosmic_alt.tiff"), units="in", width=8, height=10, res=300)
Figure_4_alt
dev.off()
...
tcga_rna %>% distinct(Samples, ClusterK4_kmeans) %>% group_by(ClusterK4_kmeans) %>% tally()
...
tcga_10count_rna <- tcga_10count_rna %>% 
  mutate(perc = ifelse(ClusterK4_kmeans == "C1.MES", round((n/52)*100),
                       ifelse(ClusterK4_kmeans == "C5.PRO", round((n/22)*100),
                              ifelse(ClusterK4_kmeans == "C2.IMM", round((n/32)*100),
                                     ifelse(ClusterK4_kmeans == "C4.DIF", round((n/53)*100))))),
         study = "TCGA-W") 
...
Freq_comparison_plot_rna_C1MES <- 
  count_10_rna %>%
  filter(ClusterK4_kmeans == "C1.MES" & eitiology %in% c("Age", "Artifact", "HRD", "Chemotherapy", "APOBEC", "POLE", "UV", "Immunosuppressants", "BER", "Tobacco", "AA", "Colibactin")) %>%
  mutate(eitiology = factor(eitiology, levels = c("Age", "Artifact", "HRD", "Chemotherapy", "APOBEC", "POLE", "UV", "Immunosuppressants", "BER", "Tobacco", "AA", "Colibactin"))) %>%
...
Freq_comparison_plot_rna_PRO <- 
  count_10_rna %>% 
  filter(ClusterK4_kmeans == "C5.PRO" & eitiology %in% c("Age", "Artifact", "HRD", "Chemotherapy", "APOBEC", "POLE", "UV", "Immunosuppressants", "BER", "Tobacco", "AA", "Colibactin")) %>%
  mutate(eitiology = factor(eitiology, levels = c("Age", "Artifact", "HRD", "Chemotherapy", "APOBEC", "POLE", "UV", "Immunosuppressants", "BER", "Tobacco", "AA", "Colibactin"))) %>%
  ggplot(., aes(x = eitiology, y = perc)) +
  geom_bar_pattern(position="dodge", stat="identity", aes(fill = eitiology, pattern=study),
                   pattern_spacing = 0.01, pattern_fill="white", pattern_size = 0.5, pattern_density=0.5, pattern_linetype = 0.5) + 
  scale_fill_manual(values = vc_cols) + 
  scale_pattern_manual(labels=c('Schildkraut-B (n=40)', 'TCGA-W (n=22)'), values=c('none', 'stripe')) +
  theme_bw() +
...
Freq_comparison_plot_rna_immuno <- 
  count_10_rna %>% 
  filter(ClusterK4_kmeans == "C2.IMM" & eitiology %in% c("Age", "Artifact", "HRD", "Chemotherapy", "APOBEC", "POLE", "UV", "Immunosuppressants", "BER", "Tobacco", "AA", "Colibactin")) %>%
  mutate(eitiology = factor(eitiology, levels = c("Age", "Artifact", "HRD", "Chemotherapy", "APOBEC", "POLE", "UV", "Immunosuppressants", "BER", "Tobacco", "AA", "Colibactin"))) %>%
  ggplot(., aes(x = eitiology, y = perc)) +
  geom_bar_pattern(position="dodge", stat="identity", aes(fill = eitiology, pattern=study),
                   pattern_spacing = 0.01, pattern_fill="white", pattern_size = 0.5, pattern_density=0.5, pattern_linetype = 0.5) + 
  scale_fill_manual(values = vc_cols) + 
  scale_pattern_manual(labels=c('Schildkraut-B (n=56)', 'TCGA-W (n=32)'), values=c('none', 'stripe')) +
  theme_bw() +
  theme(axis.text.x = element_blank(), 
        axis.text.y = element_text(color = "black", size = 10), 
        axis.title.y = element_text(color = "black", size = 10, hjust = .5, face = "bold"), 
...
Freq_comparison_plot_rna_diff <- 
  count_10_rna %>% 
  filter(ClusterK4_kmeans == "C4.DIF" & eitiology %in% c("Age", "Artifact", "HRD", "Chemotherapy", "APOBEC", "POLE", "UV", "Immunosuppressants", "BER", "Tobacco", "AA", "Colibactin")) %>%
  mutate(eitiology = factor(eitiology, levels = c("Age", "Artifact", "HRD", "Chemotherapy", "APOBEC", "POLE", "UV", "Immunosuppressants", "BER", "Tobacco", "AA", "Colibactin"))) %>%
  ggplot(., aes(x = eitiology, y = perc)) +
  geom_bar_pattern(position="dodge", stat="identity", aes(fill = eitiology, pattern=study),
                   pattern_spacing = 0.01, pattern_fill="white", pattern_size = 0.5, pattern_density=0.5, pattern_linetype = 0.5) + 
  scale_fill_manual(values = vc_cols) + 
  scale_pattern_manual(labels=c('Schildkraut-B (n=2)', 'TCGA-W (n=53)'), values=c('none', 'stripe')) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, color = "black", size = 10), 
...
Figure5a <- ggarrange(Freq_comparison_plot_rna_C1MES + rremove("ylab"), 
...
Figure5a <- annotate_figure(Figure5a, left = textGrob(expression(bold("Percent With Signature")), rot = 90, vjust = 1, hjust = 0, gp = gpar(cex = 1, fontsize = 10)))
...
tmb_perc_tissue <- 
  rna_plot %>% 
  filter(Study == "Schildkraut-B") %>%
  mutate(ClusterK4_kmeans = recode(ClusterK4_kmeans, "C1.MES" = "C1.MES n=45", "C5.PRO" = "C5.PRO n=40", "C2.IMM" = "C2.IMM n=56", "C4.DIF" = "C4.DIF n=2")) %>%
  mutate(eitiology = ifelse(is.na(eitiology), "Not Available", eitiology),
         eitiology = ifelse(eitiology == "Lymphoid", "Not Available", eitiology),
         eitiology = factor(eitiology, levels = c("Not Available", "Tobacco", "BER", "Immunosuppressants", "UV", "POLE", "APOBEC", "Chemotherapy", "HRD", "Artifact", "Age")), 
         ClusterK4_kmeans = factor(ClusterK4_kmeans, levels = c("C4.DIF n=2", "C2.IMM n=56", "C5.PRO n=40", "C1.MES n=45"))) %>%
...
rna_plot %>% filter(Study == "TCGA-W") %>% pull(ClusterK4_kmeans) %>% unique()

tmb_perc_tcga <- 
  rna_plot %>% 
  filter(Study == "TCGA-W") %>%
  mutate(ClusterK4_kmeans = recode(ClusterK4_kmeans, "C1.MES" = "C1.MES n=52", "C5.PRO" = "C5.PRO n=22", 
                                   "C2.IMM" = "C2.IMM n=32", "C4.DIF" = "C4.DIF n=53")) %>%
  mutate(eitiology = ifelse(is.na(eitiology), "Not Available", eitiology),
         eitiology = ifelse(eitiology == "Lymphoid", "Not Available", eitiology),
         eitiology = factor(eitiology, levels = c("Not Available", "Tobacco", "BER", "Immunosuppressants", "UV", "POLE", "APOBEC", "Chemotherapy", "HRD", "Artifact", "Age")),
         ClusterK4_kmeans = factor(ClusterK4_kmeans, levels = c("C4.DIF n=53", "C2.IMM n=32", "C5.PRO n=22", "C1.MES n=52"))) %>%
  ggplot(., aes(x = perc, y = ClusterK4_kmeans)) +
  geom_bar_pattern(position="stack", stat="identity", aes(fill = eitiology, pattern=Study),
                   pattern_spacing = 0.009, pattern_fill="white", pattern_size = 0.5, pattern_density=0.2, pattern_linetype = 0.5) + 
  scale_fill_manual(values = vc_cols) + 
...
tmb_perc_tissue <- tmb_perc_tissue + labs(title = "Schildkraut-B") + theme(plot.title = element_text(size = 10, face = "bold")) + rremove("ylab")
tmb_perc_tcga <- tmb_perc_tcga + labs(title = "TCGA-W") + theme(plot.title = element_text(size = 10, face = "bold")) 
Figure5b <- ggarrange(tmb_perc_tissue + rremove("xlab"), tmb_perc_tcga + rremove("xlab"))
Figure5b <- annotate_figure(Figure5b, bottom = textGrob(expression(bold("TMB %")), vjust = 1, hjust = 0, gp = gpar(cex = 1, fontsize = 10)))
...
plot_cosmic_updated <- function(df_long, exclude_signatures, signatures_list)
{
df_prepare <- 
  df_long %>%
  filter(!signature %in% {{exclude_signatures}} & eitiology %in% {{signatures_list}}) %>%
...
tissue_rna <- tissue_rna %>% filter(!signature %in% c('SBS6', 'SBS14', 'SBS15', 'SBS20', 'SBS21', 'SBS26', 'SBS44'))
tcga_rna <- tcga_rna %>% filter(!signature %in% c('SBS6', 'SBS14', 'SBS15', 'SBS20', 'SBS21', 'SBS26', 'SBS44'))


tissue_rna <- tissue_rna %>% mutate(ClusterK4_kmeans = factor(ClusterK4_kmeans, levels = c("C1.MES", "C5.PRO", "C2.IMM", "C4.DIF")))
tissue_rna <- tissue_rna %>% filter(!is.na(ClusterK4_kmeans))
tcga_rna <- tcga_rna %>% mutate(ClusterK4_kmeans = factor(ClusterK4_kmeans, levels = c("C1.MES", "C5.PRO", "C2.IMM", "C4.DIF")))
tcga_rna <- tcga_rna %>% filter(!is.na(ClusterK4_kmeans))

tmb_counts_rna_tissue <- plot_cosmic_updated(tissue_rna, signatures_missing_tcga, all_signatures) + theme(plot.margin = margin(1, 0.5, 0.5, 0.5, "cm"))
tmb_counts_rna_tcga <- plot_cosmic_updated(tcga_rna, signatures_missing_tcga, all_signatures) + theme(plot.margin = margin(1, 0.5, 0.5, 0.5, "cm"))

Figure5c <- ggarrange(tmb_counts_rna_tissue, tmb_counts_rna_tcga, labels = c("Schildkraut-B", "TCGA-W"), nrow = 2, hjust = 0)
...
Figure5c <- ggarrange(tmb_counts_rna_tissue + rremove("ylab") + rremove("xlab"), 
                      tmb_counts_rna_tcga + rremove("ylab") + rremove("xlab"), 
                      nrow = 2)
Figure5c <- annotate_figure(Figure5c, left = textGrob(expression(bold("Mutation Count")), rot = 90, vjust = 1, gp = gpar(cex = 1.3, fontsize = 8)),
                    bottom = textGrob(expression(bold("Samples")), gp = gpar(cex = 1.3, fontsize = 8)))
...
tiff(file.path(directory, "results/figures/Figure5.tiff"), units="in", width=8, height=10, res=300)

ggarrange(Figure5a, Figure5b, Figure5c, nrow = 3, heights = c(3, 1, 3), labels = c("A", "B", "C"))
