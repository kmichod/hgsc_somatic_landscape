...
qc_stats_black_cases <- qc_stats_black_cases %>% mutate(qc_coverage_pass = factor(ifelse(`Coverage at 0.95 of Target BPs` > 11, "PASS", "FAIL"))) #Define variable for whether coverage at 95% was greater than 12X
qc_stats_black_cases <- qc_stats_black_cases %>% mutate(., `Concordance Check`=recode(`Concordance Check`, 'FAIL, OK'='PASS', 'OK'='PASS')) #Recode concordance check so that the ok = pass - confirmed with david 
qc_stats_black_cases <- qc_stats_black_cases %>% mutate(., `qc_pass_overall`= as.factor(ifelse(qc_coverage_pass == "PASS" & `Concordance Check` == "PASS", "PASS", "FAIL"))) #Define overall pass variable which is pass if both qc coverage and concordance check are pass, otherwise they fail

#Define indicator variable for duplicate cases, ie cases that were resequnced
...
duplicate_suids <- c(as.character(unique(qc_stats_black_cases_duplicate$SUID))) #these are the suids for individuals who were re-sequenced n=214 following the full set of sequencing n=271

#Define sample IDs for pass and fail as lists of factors
duplicates_df <- as.data.frame(qc_stats_black_cases_duplicate %>% group_by(SUID, qc_pass_overall) %>% summarise(count=n()))
...
fail_sample_qc_tumor <- append(suids_duplicate_failed, suids_not_duplicate_failed) #suids for people failing tumor WES qc
...
normal <- normal %>% mutate(qc_coverage_pass = as.factor(ifelse(`Coverage at 0.9 of Target BPs` > 11, "PASS", "FAIL"))) #Define variable for whether coverage at 90% was greater than 12X
