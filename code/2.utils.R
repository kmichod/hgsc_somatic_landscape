#HGSC somatic landscape utilities
...
#Define filtered for false positives, these are genes that are notorious for false positives including olfactory receptors, defined based on Aaron Atkison list, list defined based on Hugo Symbol and RS number
...
tissue_epi_cosmic %>% pull(stage) %>% as.factor() %>% summary() #20 stage I cases 
tissue_epi_cosmic <- tissue_epi_cosmic %>% filter(stage != 1) #Remove stage I cases to match TCGA inclusion 
...
tissue_black_epi_final <- tissue_epi_cosmic %>% filter(!suid %in% endo_without_tp53_ids) #Remove endo cases without TP53 mutations from epi data 
tissue_black_suids_final <- unique(tissue_black_epi_final$suid) 
length(tissue_black_suids_final) #final analytic cases 191
df <- data_frame(tissue_black_suids_final)
writexl::write_xlsx(df, "/Users/kayleighlawson-michod/Desktop/tumor_WES_analytic.xlsx")

# Filter somatic mutation data to include only the final 191 analytic samples and format dataframe
tissue_black_somatic_mutations <- hgsc_38_high_confidence %>% 
...
#remove non-analytic tcga files 
