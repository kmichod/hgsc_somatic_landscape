...
somatic <- rbind(distinct_mut_tissue, distinct_mut_tcga)
...
epi <- rbind(tissue_epi, tcga_epi) 

###RNA
#Read in Waypipeline cluster assignments
rnaseq_raw <- read_csv("/Users/kayleighlawson-michod/Library/CloudStorage/OneDrive-UniversityofUtah/Doherty/AACES_Tissue_Grant/1.Analyses/Data/TCGA/FullClusterMembership.csv") %>% 
  filter(Dataset %in% c("aaces.eset", "TCGA")) %>% 
...
rnaseq_rename <- rnaseq_raw %>% 
  mutate(sample_id = ifelse(Dataset == "TCGA", gsub("\\.", "-", `...1`), 
                           ifelse(Dataset == "aaces.eset", gsub("^Sample_", "", `...1`), `...1`))) %>%
